...
func NewAnalyzeCmd(log logr.Logger) *cobra.Command {
	...
	analyzeCommand := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if analyzeCmd.overrideProviderSettings == "" {
				...
				providersMap, err = setProviderInitInfo(foundProviders)
				if err != nil {
				...
				_, err = analyzeCmd.RunProviders(cmd.Context(), containerNetworkName, containerVolName, providersMap, 5)
				if err != nil {
					log.Error(err, "failed to run provider")
					return err
				}
				err = analyzeCmd.RunAnalysis(cmd.Context(), xmlOutputDir, containerVolName, providersMap)
				if err != nil {
			...
			err = analyzeCmd.GenerateStaticReport(cmd.Context(), providersMap)
			if err != nil {
	...
	analyzeCommand.Flags().StringArrayVar(&analyzeCmd.provider, "provider", []string{}, "set provider to use")
...
func (a *analyzeCommand) setProviders(components []model.Component, foundProviders []string) ([]string, error) {
	...
	for _, c := range components {
		...
		for _, l := range c.Languages {
			if l.Name == "C#" {
				foundDotNetProv, err := setDotNetProvider(l.Frameworks)
				if err != nil {
					return nil, err
				}
				foundProviders = append(foundProviders, foundDotNetProv)
			} else {
...
func setDotNetProvider(frameworks []string) (string, error) {
	if len(frameworks) > 0 {
		for _, f := range frameworks {
			if f == "v4.5" || f == "v4.6" || f == "v4.7" || f == "v4.8" {
				return dotNetFrameworkProvider, nil
			} else {
				return dotNetProvider, nil
			}
		}
	}
	return "", fmt.Errorf("unable to set dotnet provider")
}

func setProviderInitInfo(foundProviders []string) (map[string]ProviderInit, error) {
	...
	for _, prov := range foundProviders {
		...
		switch prov {
		case javaProvider:
			providers[javaProvider] = ProviderInit{
				port:  port,
				image: Settings.JavaProviderImage,
			}
		case goProvider:
			providers[goProvider] = ProviderInit{
				port:  port,
				image: Settings.GenericProviderImage,
			}
		case pythonProvider:
			providers[pythonProvider] = ProviderInit{
				port:  port,
				image: Settings.GenericProviderImage,
			}
		case nodeJSProvider:
			providers[nodeJSProvider] = ProviderInit{
				port:  port,
				image: Settings.GenericProviderImage,
			}
			// TODO
			// case dotNetProvider, dotNetFrameworkProvider:
			// 	providers[nodeJSProvider] = ProviderInit{
			// 		port:  port,
			// 		image: "",
			// 	}
...
func (a *analyzeCommand) validateProviders(providers []string) error {
	...
	for _, prov := range providers {
		...
		if !slices.Contains(validProvs, prov) {
			return fmt.Errorf("provider %v not supported. Use --providerOverride option", prov)
		}
	}
	return nil
}
...
func (a *analyzeCommand) getConfigVolumes(providers map[string]ProviderInit) (map[string]string, error) {
	...
	javaConfig := provider.Config{
		Name:    javaProvider,
		Address: fmt.Sprintf("0.0.0.0:%v", providers[javaProvider].port),
	...
	goConfig := provider.Config{
		Name:    goProvider,
		Address: fmt.Sprintf("0.0.0.0:%v", providers[goProvider].port),
		InitConfig: []provider.InitConfig{
			{
	...
	pythonConfig := provider.Config{
		Name:    pythonProvider,
		Address: fmt.Sprintf("0.0.0.0:%v", providers[pythonProvider].port),
		InitConfig: []provider.InitConfig{
			{
	...
	nodeJSConfig := provider.Config{
		Name:    nodeJSProvider,
		Address: fmt.Sprintf("0.0.0.0:%v", providers[nodeJSProvider].port),
		InitConfig: []provider.InitConfig{
			{
				AnalysisMode: provider.SourceOnlyAnalysisMode,
				ProviderSpecificConfig: map[string]interface{}{
...
func (a *analyzeCommand) retryProviderContainer(ctx context.Context, networkName string, volName string, providers map[string]ProviderInit, retry int) error {
	if retry == 0 {
...
func (a *analyzeCommand) RunProviders(ctx context.Context, networkName string, volName string, providers map[string]ProviderInit, retry int) (map[string]ProviderInit, error) {
	runFirstProv := false
	for prov, inits := range providers {
		...
		args := []string{fmt.Sprintf("--port=%v", inits.port)}
		...
		if !runFirstProv {
			...
			err := con.Run(
				ctx,
				container.WithImage(inits.image),
				container.WithLog(a.log.V(1)),
				container.WithVolumes(volumes),
				container.WithContainerToolBin(Settings.PodmanBinary),
				container.WithEntrypointArgs(args...),
		...
		if runFirstProv && len(providers) > 1 {
...
func (a *analyzeCommand) RunAnalysis(ctx context.Context, xmlOutputDir string, volName string, providersMap map[string]ProviderInit) error {
	volumes := map[string]string{
		// application source code
		volName: SourceMountPath,
		// output directory
	...
	configVols, err := a.getConfigVolumes(providersMap)
	if err != nil {
	...
	if _, ok := providersMap[javaProvider]; !ok {
		a.enableDefaultRulesets = false
	}
	if a.enableDefaultRulesets {
		args = append(args,
	...
	// as of now only java has dep capability
	if _, ok := providersMap[javaProvider]; ok {
		// need a smarter way of handling this with a java provider + other provider
...
func (a *analyzeCommand) GenerateStaticReport(ctx context.Context, provMap map[string]ProviderInit) error {
	...
	if _, ok := provMap[javaProvider]; ok {
		if len(provMap) == 1 && a.mode == string(provider.FullAnalysisMode) {
