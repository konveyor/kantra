	...
	tests := []struct {
		...
		},
		{
			...
			setupFunc: func() (string, func(), error) {
				...
				profilesDir := filepath.Join(tmpDir, ProfilePath)
...
func TestNewHubClient(t *testing.T) {
	...
	if client.host != "http://localhost:8080" {
		t.Errorf("Expected default host to be 'http://localhost:8080', got %s", client.host)
	}
	...
	customHost := "https://example.com:9090/"
	os.Setenv("HOST", customHost)
...
func TestDeleteTarFile(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err = deleteTarFile(filePath, log)
