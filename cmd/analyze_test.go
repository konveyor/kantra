...
func Test_analyzeCommand_passProxyEnvToContainer(t *testing.T) {
	tests := []struct {
		name               string
		envVars            map[string]string
		expectedEnvPassed  []string
		unexpectedEnvPassed []string
	}{
		{
			name: "all proxy env vars set should be passed",
			envVars: map[string]string{
			...
			expectedEnvPassed: []string{"HTTP_PROXY", "HTTPS_PROXY", "NO_PROXY", "ALL_PROXY", "http_proxy", "https_proxy", "no_proxy", "all_proxy"},
		},
		{
			name: "only some proxy env vars set should pass only those",
			envVars: map[string]string{
				"HTTP_PROXY": "http://proxy.example.com:8080",
				"no_proxy":   "localhost,127.0.0.1",
			},
			expectedEnvPassed:   []string{"HTTP_PROXY", "no_proxy"},
			unexpectedEnvPassed: []string{"HTTPS_PROXY", "NO_PROXY", "ALL_PROXY", "http_proxy", "https_proxy", "all_proxy"},
		},
		{
			name:                "no proxy env vars set should pass none",
			envVars:             map[string]string{},
			expectedEnvPassed:   []string{},
			unexpectedEnvPassed: []string{"HTTP_PROXY", "HTTPS_PROXY", "NO_PROXY", "ALL_PROXY", "http_proxy", "https_proxy", "no_proxy", "all_proxy"},
		},
		{
			name: "empty proxy env vars should not be passed",
			envVars: map[string]string{
				"HTTP_PROXY":  "",
				"HTTPS_PROXY": "https://proxy.example.com:8080",
				"NO_PROXY":    "",
			...
			expectedEnvPassed:   []string{"HTTPS_PROXY"},
			unexpectedEnvPassed: []string{"HTTP_PROXY", "NO_PROXY", "ALL_PROXY", "http_proxy", "https_proxy", "no_proxy", "all_proxy"},
		},
		{
			name: "mixed case and non-proxy env vars should not interfere",
			envVars: map[string]string{
				"HTTP_PROXY":  "http://proxy.example.com:8080",
				"PATH":        "/usr/bin:/bin",
				"HOME":        "/home/user",
				"Http_Proxy":  "should not be passed",
				"http_proxy":  "http://proxy.example.com:8080",
				"ALL_PROXY":   "http://proxy.example.com:8080",
			},
			expectedEnvPassed:   []string{"HTTP_PROXY", "http_proxy", "ALL_PROXY"},
			unexpectedEnvPassed: []string{"HTTPS_PROXY", "NO_PROXY", "https_proxy", "no_proxy", "all_proxy", "PATH", "HOME", "Http_Proxy"},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, proxyVar := range proxyVars {
				originalEnv[proxyVar] = os.Getenv(proxyVar)
				os.Unsetenv(proxyVar)
			}
			defer func() {
				for _, proxyVar := range proxyVars {
					if val, exists := originalEnv[proxyVar]; exists && val != "" {
			...
			// Mock container runner to capture env vars
			capturedEnvVars := make(map[string]string)
			
			// Test the proxy env collection logic from RunAnalysis method (lines 1194-1199)
			for _, proxyVar := range proxyVars {
				if value := os.Getenv(proxyVar); value != "" {
					capturedEnvVars[proxyVar] = value
				}
			}

			// Verify expected env vars are captured
			for _, expectedVar := range tt.expectedEnvPassed {
				if _, exists := capturedEnvVars[expectedVar]; !exists {
					t.Errorf("Expected env var %s to be captured but it wasn't", expectedVar)
				}
			}

			// Verify unexpected env vars are not captured
			for _, unexpectedVar := range tt.unexpectedEnvPassed {
				if _, exists := capturedEnvVars[unexpectedVar]; exists {
					t.Errorf("Unexpected env var %s was captured", unexpectedVar)
				}
			}

			// Verify the values match for captured vars
			for key, expectedValue := range tt.envVars {
				if expectedValue != "" {
					if capturedValue, exists := capturedEnvVars[key]; exists {
						if capturedValue != expectedValue {
							t.Errorf("Env var %s = %v, want %v", key, capturedValue, expectedValue)
						}
					}
				}
			}
...
func Test_analyzeCommand_proxyEnvVarsLogic(t *testing.T) {
	tests := []struct {
		name     string
		envVars  map[string]string
		expected int // number of env vars that should be passed
	}{
		{
			name: "all eight proxy vars set",
			envVars: map[string]string{
		...
		{
			name: "empty values should not count",
			envVars: map[string]string{
				"HTTP_PROXY":  "",
				"HTTPS_PROXY": "https://proxy:8080",
				"NO_PROXY":    "",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Simulate the logic from RunAnalysis lines 1194-1199
			var envCount int
			for _, proxyVar := range proxyVars {
				if value := os.Getenv(proxyVar); value != "" {
					envCount++
			...
			if envCount != tt.expected {
				t.Errorf("Expected %d proxy env vars to be processed, got %d", tt.expected, envCount)
			}
		})
	}
}
