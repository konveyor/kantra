...
func TestDefaultRulesetPathsForProvidersDedup(t *testing.T) {
	tmpDir := t.TempDir()
	if err := os.MkdirAll(filepath.Join(tmpDir, "java"), 0o755); err != nil {
		t.Fatal(err)
	}
	...
	got := DefaultRulesetPathsForProviders(tmpDir, providers)
	if len(got) != 1 {
		t.Errorf("got %v, want single java path", got)
	}
}

func TestBundledDefaultRulesetSubdir(t *testing.T) {
	tests := []struct {
		provider string
		want     string
	}{
		{util.JavaProvider, "java"},
		{util.NodeJSProvider, "nodejs"},
		{util.CsharpProvider, "dotnet"},
		{util.GoProvider, ""},
		{util.PythonProvider, ""},
		{"builtin", ""},
	}
	for _, tt := range tests {
		if got := BundledDefaultRulesetSubdir(tt.provider); got != tt.want {
			t.Errorf("BundledDefaultRulesetSubdir(%q) = %q, want %q", tt.provider, got, tt.want)
		}
	}
}
